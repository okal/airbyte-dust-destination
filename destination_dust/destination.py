...
class DestinationDust(Destination):
    ...
    def _ensure_table_exists(
        ...
        Args:
            ...
            table_id: Optional table ID. If None, Dust will generate one.
            schema: Column schema dictionary
            configured_stream: Configured stream (unused, kept for compatibility)
            
        Returns:
            The table_id (either the provided one or the one generated by Dust)
        """
        ...
        response = client.upsert_table(
            ...
            table_id=table_id,  # None if we want Dust to generate it
        )
        
        # Extract table_id from response if Dust generated it
        if table_id is None:
